...
| --- | --- | --- | --- |
| [授权加密桌面端子方案](./RFID_M1授权加密桌面端子方案.md) | 云平台账号登录、客户注册与查询、墨盒授权、储值、最小状态提示、读卡器通信，以及喷码机配置操作入口。 | 云平台会话和客户接口集成、固定版本客户密钥库、每卡密钥派生，以及统一密钥材料获取接口。 | 云平台账号可登录、客户可注册；与真实读卡器完成授权和储值并在本地验收；结果不确定时停止墨盒流转；明文密钥不被持久化。 |
| [喷码机 App 端子方案](./RFID_M1喷码机App端子方案.md) | 组装/维护模式下的受保护配置、密钥安全保存、现场离线验证墨盒、喷码准入、成功后扣减与失败锁定。 | 通过统一密钥材料获取接口提供客户 `Key A`；不创建配置任务或保存配置结果。 | 只有本客户合法且余额充足的墨盒可喷码；成功回调后余额恰好减 1；扣减异常时禁止下一次喷码。 |
...
| 第 6 节：授权、储值及密钥后端 | 桌面端子方案。 |
| 第 7 节：喷码机出厂配置 | 跨端联调；桌面端负责取得并传递客户 `Key A`，喷码机 App 端子方案负责受保护接口和安全存储。 |
...
桌面端 <-> 后端：注册/查询客户、取得短时密钥材料
桌面端 <-> 读卡器：RFID 命令、UID、卡块和余额（明文密钥仅在进程内存）
后端 -> 桌面端 -> 喷码机：客户 Key A；客户编号、密钥版本和布局版本由桌面端组成配置材料
喷码机 <-> 墨盒：离线认证、校验、读值、减值
```

云平台是操作员账号和客户主数据的来源；RFID 后端只保存客户映射、客户密钥和工厂根密钥，并按需返回短时密钥材料；桌面端是墨盒编号生成、人工操作、读写卡和本地验收执行者；喷码机是离线消费次数的唯一执行者。喷码机配置接口由喷码机方实现，配置动作及结果只在桌面端与喷码机之间本地处理，不上传后端。
...
后端不保存 `cartridge_id`、卡 UID、授权结果、储值前后余额或喷码机配置结果。
...
`cartridge_id` 必须由桌面端以密码学安全随机数生成，并在工位流程中禁止主动复用。后端不保存墨盒编号，因此不能通过数据库唯一约束提供全局去重；F08 的 UID 可能被复制，不能用它代替墨盒编号或作为唯一防伪依据。
...
- 授权模块：选择客户和初始次数，本地生成 `cartridge_id`，取得密钥材料，初始化卡片并在本地完成回读验收。
- 储值模块：选择客户并先取得 `Key A`，认证后读取卡内 `customer_id`、`cartridge_id`，再取得该卡 `Key B`，执行加值并在本地核对前后余额。
- 共用能力：云平台账号登录、客户注册与查询、读卡器通信、喷码机受控配置入口，以及成功、失败或结果不确定的最小状态提示。
- 云平台负责操作员账号和客户主数据；RFID 后端负责客户映射、客户密钥和工厂根密钥，不保存授权、储值或喷码机配置结果。第一版不提供密钥版本变更、操作记录查询、异常恢复或软件隔离功能。

### 6.2 运行与密钥边界

授权、储值桌面端运行在受控工位，并连接读卡器完成本方案所需的 RFID 指令。数据、客户密钥密文和工厂根密钥存储在后端；桌面端不持久化这些密钥。

桌面端调用统一密钥材料接口：传入 `customer_id` 时返回客户 `Key A`；同时传入 `cartridge_id` 时再返回按既定规则派生的每卡 `Key B`。授权直接以本地生成的 `cartridge_id` 取得两把密钥；储值先只取得 `Key A` 读取卡内编号，再带 `cartridge_id` 取得 `Key B`；喷码机配置只取得 `Key A`。密钥仅可保存在进程运行内存中；完成、取消、超时、注销或应用退出时清除。禁止写入本地持久化存储、缓存、下载文件、埋点、错误上报或客户端日志。
...
| 获取密钥材料 | `POST /api/rfid/key-material` | 请求 `customer_id` 和可选 `cartridge_id`；响应 `customer_id`、固定 `key_version = 1`、客户 `Key A`，并在提供 `cartridge_id` 时响应派生的每卡 `Key B` | 仅允许有效且具备权限的云平台会话调用；HTTPS 传输；`Cache-Control: no-store`；禁止缓存、记录请求/响应体或返回工厂根密钥。 |

删除授权创建/完成、储值创建/完成以及喷码机配置创建/取材料/提交结果共 7 个接口。后端不接收或保存授权、储值、喷码机配置结果；这些操作均由桌面端通过 RFID 回读或喷码机响应在本地验证。云平台客户注册成功但本地映射或密钥记录创建失败时，仍使用 `cloud_customer_id` 重试本地初始化，不得再次注册客户。

储值不得改变 `cartridge_id`、客户归属、密钥版本、身份块、元数据块或扇区尾块，只能改变块 5 的余额。桌面端必须在单次操作内防止重复下发加值命令；后端不保存未完成储值状态，因此应用重启后无法通过后端阻止对结果不确定墨盒再次储值，必须继续执行贴标并退出流转的人工规程。

### 6.4 授权本地状态

```text
桌面端生成 cartridge_id 并取得短时密钥材料
  -> 桌面端使用密钥材料写卡
  -> 本地回读验收通过 -> 授权成功
  -> 不可逆写入后中断/验收失败 -> 结果不确定 -> 墨盒贴标并退出流转
```

桌面端先本地生成 `cartridge_id` 并取得 `Key A`、`Key B`，只有本地回读身份资料和值块次数均匹配后才显示授权成功。块 7 写入后若桌面端、读卡器中断或验收失败，桌面端提示结果不确定；操作员必须将墨盒贴标并退出发货流程。第一版不提供软件恢复或隔离入口，本次使用的 `cartridge_id` 不得主动复用。

### 6.5 授权完整流程图

```mermaid
flowchart TD
    A[管理员选择客户和初始次数] --> B[桌面端本地生成墨盒编号]
    B --> C[桌面端向后端取得<br/>客户密钥和派生的每卡管理密钥]
    ...
    N -- 是 --> Q[本地验收通过<br/>显示授权成功]
    N -- 否 --> U
    T --> D
```

图中“写入后失败或中断”包括桌面端和读卡器中断。业务密钥和访问权限设置是不可逆切换点：设置前允许在当前操作内重新写卡；设置后若无法完成本地验收，只能将墨盒贴标并退出流转，不得复用本次墨盒编号。

### 6.6 单张墨盒授权流程

1. 管理员选择目标客户和初始可喷码次数；桌面端本地生成 64 位随机 `cartridge_id`，调用密钥材料接口取得客户 `Key A`、固定 `key_version = 1` 和该卡派生 `Key B`。
2. 桌面端仅在当前操作内存中保留密钥材料；管理员连接读卡器。
3. 读卡器寻卡并读取 UID。
4. 桌面端使用取得的客户 `Key A` 和派生的该卡 `Key B`。
5. 使用运输状态的默认密钥认证扇区 1；写入块 4 和块 6。
6. 以 `CMD_INITVAL_CLASS` 将块 5 初始化为初始次数。
7. 最后写入块 7：客户 `Key A`、访问控制位和派生 `Key B`。必须最后写入，避免中途失败使卡片不可恢复。
8. 分别用 `Key A` 和 `Key B` 重新认证，读取块 4、块 6 并用 `CMD_READVAL_CLASS` 校验块 5；全部匹配后在本地显示授权成功。

授权中任一步失败时不得显示授权成功。写入块 7 之前失败可在当前操作内重新初始化；块 7 已写入但验收失败或结果不确定时，桌面端不提供恢复入口，操作员必须将墨盒贴标并退出发货流程，本次 `cartridge_id` 不得主动复用。

### 6.7 单张墨盒储值流程

1. 管理员选择客户、输入增加次数并将墨盒放到读卡器；桌面端寻卡并读取 UID。
2. 桌面端以所选 `customer_id` 调用密钥材料接口取得 `Key A`，用 `Key A` 认证扇区 1，读取块 4、块 6，校验客户归属后取得卡内 `cartridge_id`。
3. 桌面端以 `customer_id` 和卡内 `cartridge_id` 再次调用同一接口取得派生 `Key B`；增加次数只保存在当前本地操作中。
4. 桌面端以 `Key B` 重新认证，读取块 4、块 6 和块 5；校验 UID、魔数、格式版本、`cartridge_id`、客户归属、密钥版本及 CRC32。
5. 使用 `CMD_INC_CLASS` 对块 5 加值指定次数，再使用 `CMD_READVAL_CLASS` 回读余额。桌面端不得直接写块 5，也不得写入块 4、块 6 或块 7。
6. 桌面端在本地确认余额增量准确匹配本次输入、其他资料未改变后显示储值成功。

加值后发生中断、移卡或回读失败时，桌面端提示“结果不确定”，操作员必须将墨盒贴标并退出流转。后端不保存未完成状态，第一版也不提供查询、恢复或解除入口。

### 6.8 墨盒储值完整流程图

```mermaid
flowchart TD
    A[管理员选择客户并输入增加次数] --> B[桌面端读取卡片序列号]
    B --> C[按所选客户取得客户认证密钥]
    C --> D[用客户认证密钥读取并校验<br/>卡内客户编号和墨盒编号]
    D --> E[带客户和墨盒编号<br/>取得派生的卡片管理密钥]
    E --> H[桌面端仅在当前操作内存中<br/>使用密钥和增加次数]
    H --> I[使用卡片管理密钥验证墨盒]
    ...
    N -- 是 --> O[回读墨盒资料和储值后次数]
    O --> P{墨盒资料未变化且次数增量<br/>是否与本次输入一致？}
    P -- 是 --> S[本地验收通过<br/>显示储值成功]
    P -- 否 --> T
```

储值只能增加可喷码次数，不得修改墨盒编号、客户归属、密钥版本或其他墨盒资料。加值后结果不确定时不得再次加值；该墨盒由工位人工保管或报废，不再进入业务流转。
...
该接口必须将密钥保存到受保护的持久化存储，禁止通过查询接口返回明文密钥，禁止在普通运行模式调用。桌面端只在本地验证接口响应，不将喷码机序列号、配置时间或结果上传后端。该接口不是 `RFID功能指令_v1.0.38.pdf` 已有功能，需要喷码机固件另行实现。
...
flowchart TD
    subgraph FACTORY[出厂或受控维护阶段]
        A[管理员选择喷码机和目标客户] --> B[桌面端向后端取得客户认证密钥]
        B --> D[桌面端组成客户编号、密钥版本、客户认证密钥和卡片布局信息]
...
| 正常喷码 | 成功回调后余额恰好减 1。 |
| 正常储值 | 使用 `Key B` 加值后余额恰好增加本次输入次数，其他卡片资料不变。 |
| 密钥材料接口 | 有权限会话可按客户和可选墨盒编号取得所需密钥；响应不缓存、不记录，且不创建操作记录。 |
| 储值结果不确定 | 桌面端不显示成功并提示墨盒退出流转；后端不保存或恢复该状态。 |
