...
- 桌面端通过 HTTPS 调用登录、客户、授权任务和储值任务接口；
- `Key A`、`Key B` 只允许在当前任务的进程内存中短时存在，任务结束后应主动清除，不得进入本地文件、日志、缓存、埋点或错误上报。
...
- 串口对象、协议收发、任务密钥和 HTTPS 任务编排应放在 main 或专用 utility process 中；renderer 只接收进度、非敏感卡片字段和成功/失败/结果不确定状态。
- preload 只暴露窄接口，例如“列出端口”“开始授权”“取消当前任务”，不应把原始串口对象、通用文件能力、任意 IPC 或密钥字节暴露给 renderer。
...
- 密钥应从接口响应尽早解析到 `byte[]`，通过 `Span<byte>` 使用，避免生成 `string`。任务退出的所有路径都应在 `finally` 或等效的集中清理路径调用 `CryptographicOperations.ZeroMemory`。
...
- `QNetworkAccessManager` 和 `QSerialPort` 都基于 Qt 事件模型，能够在同一应用中进行异步编排；需要清晰控制对象所属线程和任务取消路径。
...
以上措施不能抵御同权限调试器、已攻破的桌面端进程或密钥正在使用时的内存读取。现有 ADR 已将受控终端和“进程被攻破时任务密钥可能泄露”作为系统边界，技术栈选型不能改变这一事实。
...
- 在任务模拟结束后检查应用日志、崩溃报告和本地目录，确认不出现测试密钥，并对显式缓冲区执行清零。
